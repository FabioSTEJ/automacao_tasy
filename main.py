...
def aguardar_mudanca_de_fase(fase_anterior, timeout=15):
    ...
    """
    print(
        f"[AGUARDANDO] Ação para '{fase_anterior}' concluída. Esperando a tela mudar..."
    )
    start_time = time.time()
    while time.time() - start_time < timeout:
        fase_atual = verificador_sistema.identificar_fase_atual()
        if fase_atual != fase_anterior:
            print("[AGUARDANDO] Mudança de tela detectada. Prosseguindo...")
            return True
        time.sleep(0.5)  # Verifica a cada meio segundo

    print(
        f"[AVISO] Timeout: A tela permaneceu em '{fase_anterior}' por mais de {timeout} segundos."
    )
    return False
...
def rodar_robo():
    ...
    while True:
        try:
            ...
            if fase == "CARREGANDO":
                print(
                    "[SISTEMA] Tasy processando (Carregamento detectado). Aguardando..."
                )
                time.sleep(2)
                continue

            if fase not in ["DESCONHECIDO", ultima_fase]:
                print(f"--- [DETECTADO]: {fase} ---")

                if fase in acoes:
                    if sucesso := acoes[fase]():

                        ultima_fase = fase
                    else:
                        print(
                            f"[AVISO] Falha na execução de {fase}. Tentando novamente..."
                        )
                        ultima_fase = None
                else:
                ...
                if acoes_tasy.monitorar_instabilidade_autoatendimento():
                    print(
                        "[SISTEMA] Instabilidade detectada e tratada. Reiniciando ciclo de verificação."
                    )
                    ultima_fase = None
            ...
            if ultima_fase == fase and fase not in ["AUTO_ATENDIMENTO", "DESCONHECIDO"]:
                ...
                print(
                    "[ORQUESTRADOR] Fase desconhecida detectada. Iniciando protocolo de verificação."
                )
                ...
                if fase_pos_foco == "DESCONHECIDO":
                    print(
                        "[ORQUESTRADOR] Tela continua desconhecida. Aguardando estabilização (15s)..."
                    )
                    time.sleep(15)
                    ...
                    if fase_final == "DESCONHECIDO":
                        print(
                            "[ORQUESTRADOR] ERRO PERSISTENTE: Salvando evidência de erro."
                        )
